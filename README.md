...
- 🎯 Entry points to execute your program using `python -m <media_wizard>` or `$ media_wizard` with basic CLI argument parsing.
...
---
# media_wizard Flask Application

Awesome media_wizard created by dferndz
...
```bash
git clone https://github.com/dferndz/media-wizard media_wizard
cd media_wizard
make install
```
...
```bash
media_wizard create-db   # run once
media_wizard populate-db  # run once (optional)
media_wizard add-user -u admin -p 1234  # ads a user
media_wizard run
```
