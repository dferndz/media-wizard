...
original_author="dferndz"
original_name="media_wizard"
original_urlname="media-wizard"
original_description="Awesome media_wizard created by dferndz" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/media_wizard "${name}"
...
then
    ...
    echo "then run 'media_wizard' or 'python -m media_wizard'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'media_wizard' or 'python -m media_wizard' or 'poetry run media_wizard'"
fi
